...
export class CanvasRenderer {
    ...
    if (scrollbackVisible) {
      ...
      for (let i = 0; i < scrollbackRows; i++) {
        ...
        if (idx < term.scrollbackBuffer.length) {
          ...
          visibleRows.push({
            grid: sbRow,
            gridCols: sbRow.length / CELL_WORDS,
            gridY: 0,
            screenY: i,
          })
        }
  ...
  renderRowBg(grid, gridCols, gridY, screenY) {
    ...
    for (let col = 1; col <= renderCols; col++) {
      ...
      if (cellBg !== currentBg) {
        ...
        this.ctx.fillRect(
          bgStart * this.charWidth,
          baseline,
          (col - bgStart) * this.charWidth,
          this.charHeight
        )
        bgStart = col
        currentBg = cellBg
  ...
  renderBlockChar(code, x, y, color) {
    ...
    if (code === 0x2580) {
      this.ctx.fillRect(x, y, w + 0.5, Math.ceil(h / 2))
      return true
    }
    ...
    if (code === 0x2594) {
      this.ctx.fillRect(x, y, w, Math.max(1, Math.round(h / 8)))
      return true
    }
    if (code === 0x2595) {
    ...
    if (code >= 0x2596 && code <= 0x259f) {
      const masks = [0b0010, 0b0001, 0b1000, 0b1011, 0b1001, 0b1110, 0b1101, 0b0100, 0b0110, 0b0111]
