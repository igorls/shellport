...
const SECURITY_HEADERS = {
  ...
  'Content-Security-Policy':
    "default-src 'self'; script-src 'unsafe-inline'; style-src 'unsafe-inline'; connect-src 'self' ws: wss:; img-src 'self' data:; font-src 'self' data:; object-src 'none'; frame-ancestors 'none'; base-uri 'none'; form-action 'none';",
...
export async function startServer(config: ServerConfig): Promise<void> {
  ...
  if (config.totp && config.totpSecret) {
    ...
    console.log(
      '[ShellPort] 🔐 Interactive approval mode enabled (connections require manual approval)'
    )
  }
      ...
      if (url.pathname === '/ws') {
        ...
        if (
          server.upgrade(req, {
            ...
            headers: { 'Sec-WebSocket-Protocol': `shellport-v${PROTOCOL_VERSION}` },
          })
        ) {
          return
        }
    ...
    websocket: {
      ...
      async message(ws, message) {
        ...
        if (msgBuffer.byteLength > MAX_FRAME_SIZE) {
          console.log(
            `[ShellPort] ❌ Oversized frame from ${sessionData.clientIP} (${msgBuffer.byteLength} bytes) — disconnecting.`
          )
        ...
        sessionData.recvQ.add(async () => {
          ...
          if (baseKey && sessionData.serverNonce) {
            ...
            if (msgView.length < 17) {
              // 1 type byte + 16 nonce bytes
              ws.close(4003, 'Authentication failed')
              return
            }
