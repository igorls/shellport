...
> ⚠️ **For production use**, always use the default auto-generated secret and keep approval mode enabled. Use `--no-secret` only on isolated, trusted networks.
...
Browser                          Server
  ...
  │◀──────────────────────────────│
  │                                │
  │  2. Client sends nonce + proof │
  │──────────────────────────────▶│
  │  (per-session salt derived)    │──▶ Approval prompt (if enabled)
  │                                │
